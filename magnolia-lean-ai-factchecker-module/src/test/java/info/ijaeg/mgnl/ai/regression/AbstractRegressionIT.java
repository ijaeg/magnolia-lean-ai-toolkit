...
/**
 * Generische Basis für die Regressions-Testklassen. Kennt weder
 * {@link ExtractorCase} noch {@link CheckerCase} – nur ihre Subklassen
 * wissen, welchen konkreten Falltyp sie durchlaufen.
 */
abstract class AbstractRegressionIT {
    ...
    protected <T> void runAllAndAssert(List<T> cases, Consumer<T> singleCaseCheck,
        ...
        assertTrue(failures.isEmpty(),
                () -> failures.size() + " von " + cases.size() + " Fällen fehlgeschlagen:\n"
                        + String.join("\n", failures));
    }
}